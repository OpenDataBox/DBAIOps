...
def getoratbsautoext(ora):
    ...
    vres = []
    sql = '''
select  a.tablespace_name,b.AUTOEXTENSIBLE,b.file_name
 from dba_tablespaces a,dba_temp_files b
where a.contents in ('TEMPORARY')
and a.TABLESPACE_NAME=b.tablespace_name 
union all
select a.tablespace_name,b.AUTOEXTENSIBLE,b.file_name
 from dba_tablespaces a,dba_data_files b
where a.contents in ('UNDO')
and a.TABLESPACE_NAME=b.tablespace_name 
'''
    ...
    if not rs == []:
        for res in rs:
            if res[1] == "YES":
                vs += 1
                vt.append(res[0] + "表空间数据文件自动扩展未关闭，建议关闭")
            else:
                vres.append(res[0] + "表空间数据文件自动扩展关闭")

    if vs > 0:
        score += 1
        fres.append(dict(rkey="数据文件自动扩展检查", rval="\n".join(str(i) for i in vt), rtype=1, rnote="数据文件自动扩展检查不合规"))
    else:
        fres.append(dict(rkey="数据文件自动扩展检查", rval="\n".join(str(i) for i in vres), rtype=0, rnote="数据文件自动扩展检查合规"))
    return score, fres
