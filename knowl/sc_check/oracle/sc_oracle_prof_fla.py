...
"""
    ...
    if not rs == []:
        for x in rs:
            if x[3] != 5:
                vs += 1
                vt.append("Profile为：" + x[0] + "的用户登录数据库失败次数为：" + str(x[3]) + "，不合规，建议设置failed_login_attempts为5")
            else:
                vres.append("Profile为：" + x[0] + "的用户登录数据库失败次数为：5，符合规范")

    if vs > 0:
        score += 1
        fres.append(dict(rkey="密码5次过期策略检查", rval="\n".join(str(i) for i in vt), rtype=1, rnote="密码5次过期策略检查不合规"))
    else:
        fres.append(dict(rkey="密码5次过期策略检查", rval="\n".join(str(i) for i in vres), rtype=0, rnote="密码5次过期策略检查检查合规"))
    return score, fres
