...
def getoractl(ora):
    ...
    if not rs == []:
        if not (len(rs) >= 2 and len(rs) <= 3):
            vs += 1
            vt.append("控制文件个数检查不合规，要求：最少2个，最多3个")
        else:
            vres.append("控制文件个数：" + str(len(rs)) + "符合要求")

    if vs > 0:
        score += 1
        fres.append(dict(rkey="数据库控制文件个数检查", rval="\n".join(str(i) for i in vt), rtype=1, rnote="数据库控制文件个数检查不合规"))
    else:
        fres.append(dict(rkey="数据库控制文件个数检查", rval="\n".join(str(i) for i in vres), rtype=0, rnote="数据库控制文件个数检查合规"))
    return score, fres
