...
"""
    ...
    if not rs == []:
        for x in rs:
            if x[3] != 'UNLIMITED':
                vs += 1
                vt.append("Profile为：" + x[0] + "的密码过期策略未禁用，不合规，建议设置PASSWORD_LIFE_TIME为UNLIMITED")
            else:
                vres.append("Profile为：" + x[0] + "的密码过期策略已禁用，符合规范")

    if vs > 0:
        score += 1
        fres.append(dict(rkey="密码过期策略禁用检查", rval="\n".join(str(i) for i in vt), rtype=1, rnote="密码过期策略禁用检查不合规"))
    else:
        fres.append(dict(rkey="密码过期策略禁用检查", rval="\n".join(str(i) for i in vres), rtype=0, rnote="密码过期策略禁用检查合规"))
    return score, fres
