...
def getoraversion(ora):
    score = 0
    fres = []
    sql = """select distinct action, version, comments
  from (select row_number() over(order by action_time desc) rn,
               action,
               version,
               comments
 ...
 where rn = 1"""
    ...
    if not rs == []:
        for res in rs:
            vres = res[2]
    if "11.2.0.4.180417" not in vres:
        score += 1
        fres.append(dict(rkey="数据库版本检查", rval="当前数据库版本：" + vres + "，不符合规范要求的：11.2.0.4.180417", rtype=1,
                         rnote="数据库版本不合规要求PSU 11.2.0.4.180417"))
    else:
        fres.append(dict(rkey="数据库版本检查", rval="当前数据库版本：" + vres + "，符合规范", rtype=0, rnote="数据库版本合规"))
    return score, fres
