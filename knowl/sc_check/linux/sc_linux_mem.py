...
def register(file_name):
    res = []
    res.append(dict(ename='mem_size', cname='内存大小', desc='内存大小，单位：GB'))
...
def getlinuxmem():
    ...
    # if int(vres) < 524288000 and "No such file" not in vres:
    if int(vres) / 1024 / 1024 < int(param_in) and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统内存检查",
                         rval="当前系统内存：" + str(round(int(vres) / 1024 / 1024, 2)) + "GB，小于规定要求的" + param_in + "GB",
                         rtype=1, rnote="系统内存小于规定要求"))
    else:
        fres.append(dict(rkey="系统内存检查", rval="当前系统内存：" + str(round(int(vres) / 1024 / 1024, 2)) + "GB，符合要求", rtype=0,
                         rnote="系统内存合规"))
    return score, fres
