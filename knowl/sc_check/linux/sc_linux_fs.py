...
def getlinuxfs():
    ...
    vs = 0
    vt = []
    cmd = "df -P|grep -v \"Mounted\"|awk '{print $6\" \"$2}'"
    vres = helper.openCmd(cmd).strip()
    if "/boot " not in vres:
        vt.append("/boot分区不存在，且需要至少200M空间")
    if "/var " not in vres:
        vt.append("/var分区不存在，且需要至少10G空间")
    if "/tmp " not in vres:
        vt.append("/tmp分区不存在，且需要至少20G空间")
    if "/u01 " not in vres:
        vt.append("/u01分区不存在，且需要至少80G空间")

    for x in vres.split("\n"):
        if x.split(" ")[0] == "/boot":
            vs += 1
            if int(x.split(" ")[1]) < 204800:
                vs -= 1
                vt.append("/boot空间小于要求的200M")
        if x.split(" ")[0] == "/var":
            vs += 1
