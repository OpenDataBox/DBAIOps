...
def register(file_name):
    res = []
    res.append(dict(ename='disk_size', cname='磁盘可用空间大小', desc='磁盘可用空间大小，单位：GB'))
...
def getlinuxspace():
    ...
    if int(int(vres) / 1024 / 1024) < int(param_in) and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统可用磁盘空间检查",
                         rval="当前系统可用磁盘空间为：" + str(int(int(vres) / 1024 / 1024)) + "GB，不符合规范要求的" + param_in + "GB",
                         rtype=1, rnote="系统可用磁盘空间不合规"))
    else:
        fres.append(dict(rkey="系统可用磁盘空间检查",
                         rval="当前系统可用磁盘空间为：" + str(int(int(vres) / 1024 / 1024)) + "GB，符合规范要求的" + param_in + "GB",
