...
def getlinuxrpm():
    score = 0
    fres = []
    cmd = "\"rpm -q binutils compat-libcap1 compat-libstdc++-33 gcc gcc-c++ glibc glibc-devel ksh libgcc libstdc++ libstdc++-devel libaio libaio-devel libaio-devel elfutils-libelf-devel make sysstat --qf '%{name}.%{arch}\n'|sort|grep \\\"not installed\\\"\""
    vres = helper.openCmd(cmd).strip()
    if vres != "" and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统RPM包检查", rval=vres, rtype=1, rnote="系统RPM包不合规"))
    else:
        fres.append(dict(rkey="系统RPM包检查", rval="系统RPM包已全部安装", rtype=0, rnote="系统RPM包合规"))
    return score, fres
...
if __name__ == '__main__':
    ...
    try:
        s8, f8 = getlinuxrpm()
        if f8:
            # print('{"check":' + json.dumps(f8[0]) + '}')
            print('msg=' + json.dumps(f8[0]))
