...
def getlinuxselinux():
    ...
    if vres == "Enforcing" and "No such file" not in vres:
        score += 1
        fres.append(
            dict(rkey="系统Selinux检查", rval="当前系统SELinux状态为：" + vres + "，不符合规范要求，需要关闭", rtype=1, rnote="系统Selinux配置不合规"))
    else:
        fres.append(dict(rkey="系统Selinux检查", rval="当前系统SELinux状态为：" + vres + "，符合规范要求", rtype=0, rnote="系统Selinux配置合规"))
    return score, fres
...
if __name__ == '__main__':
    ...
    try:
        s10, f10 = getlinuxselinux()
        if f10:
            # print('{"check":' + json.dumps(f10[0]) + '}')
            print('msg=' + json.dumps(f10[0]))
