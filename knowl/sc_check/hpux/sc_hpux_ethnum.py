...
def register(file_name):
    res = []
    res.append(dict(ename='heth_num', cname='网卡个数', desc='网卡个数'))
...
def gethpuxethnum():
    score = 0
    fres = []
    cmd = "lanscan|grep \"UP\"|grep lan[0-9]|awk '{print $5}'|sort -u|wc -l"
    vres = helper.openCmd(cmd).strip()
    # if int(vres) < 5 and "No such file" not in vres:
    if int(vres) < int(param_in) and "No such file" not in vres:
        score += 1
        fres.append(
            dict(rkey="系统网卡个数检查", rval="当前系统网卡个数为：" + vres + "，不符合规范要求的" + param_in + "个", rtype=1, rnote="系统网卡个数小于4"))
    else:
        fres.append(
            dict(rkey="系统网卡个数检查", rval="当前系统网卡个数为：" + vres + "，符合规范要求的" + param_in + "个", rtype=0, rnote="系统网卡个数合规"))
    return score, fres
