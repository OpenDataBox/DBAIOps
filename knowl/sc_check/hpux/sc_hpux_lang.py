...
def gethpuxlang():
    score = 0
    fres = []
    # cmd = "grep LANG /etc/environment"
    cmd = "locale -a"
    vres = helper.openCmd(cmd).strip()
    if "en_US" not in vres and "zh_CN" not in vres and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统字符集检查", rval="当前系统字符集不符合规范要求的en_US.UTF-8或者zh_CN", rtype=1, rnote="系统字符集不合规"))
    else:
        fres.append(dict(rkey="系统字符集检查", rval="当前系统字符集符合规范要求的en_US.UTF-8或者zh_CN", rtype=0, rnote="系统字符集合规"))
    return score, fres
