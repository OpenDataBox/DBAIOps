...
def register(file_name):
    res = []
    res.append(dict(ename='hmem_size', cname='内存大小', desc='内存大小，单位：GB'))
...
def gethpuxmem():
    score = 0
    fres = []
    cmd = "swapinfo -m |grep \"memory\"|awk '{print $2}'"
    vres = helper.openCmd(cmd).strip()
    # if int(vres) < 512000 and "No such file" not in vres:
    if int(vres) < int(param_in) * 1024 and "No such file" not in vres:
        score += 1
        fres.append(
            dict(rkey="系统内存检查", rval="当前系统内存：" + str(round(int(vres) / 1024, 2)) + "GB，小于规定要求的" + param_in + "GB",
                 rtype=1, rnote="系统内存小于规定要求"))
    else:
        fres.append(
            dict(rkey="系统内存检查", rval="当前系统内存：" + str(round(int(vres) / 1024, 2)) + "GB，符合要求", rtype=0, rnote="系统内存合规"))
    return score, fres
