...
def getaixtmzone():
    score = 0
    fres = []
    cmd = "grep TZ /etc/environment"
    vres = helper.openCmd(cmd).strip()
    if "BEIST-8" not in vres and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统时区检查", rval="当前系统时区为：" + vres + "，不符合规范要求的BEIST-8", rtype=1, rnote="系统时区设置不合规"))
    else:
