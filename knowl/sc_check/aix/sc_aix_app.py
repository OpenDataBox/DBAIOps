...
def getaixapp():
    score = 0
    fres = []
    cmd = "lslpp -l bos.adt.base bos.adt.lib bos.adt.libm bos.perf.libperfstat bos.perf.perfstat bos.perf.proctools rsct.basic.rte rsct.compat.clients.rte xlC.aix61.rte gpfs.base|grep \"not installed\""
    vres = helper.openCmd(cmd).strip()
    if vres != "" and "No such file" not in vres:
        score += 1
        fres.append(dict(rkey="系统软件包检查", rval="当前系统如下软件包未安装：" + vres, rtype=1, rnote="系统软件包不合规"))
    else:
        fres.append(dict(rkey="系统软件包检查", rval="当前系统软件包安装合规", rtype=0, rnote="系统软件包合规"))
    return score, fres
