...
when cib_value='' then ' ' else cib_value end cib_value from p_oracle_cib where target_id='{0}' and cib_name in (
 'db_block_size') and index_id='2201010'
union
...
index_id='2201003' and target_id='{1}' and seq_id=1) as foo order by 
case when cib_name='db_name' then 1 when cib_name='database_role' then 2 when cib_name='dbid' then 3 
 ...
 when cib_name='cdb' then 17 when cib_name='pdb count' then 18 end'''.format(targetid, dbid)
    # print(sqldbinfo)
