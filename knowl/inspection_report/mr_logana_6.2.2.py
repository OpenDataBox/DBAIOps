...
def getlogresult(pg, targetid, seqno):
    ...
    sqllogresult = "select result from log_result where target_id='{0}' and seq_no='{1}'".format(targetid, seqno)
...
def getora600list(pg, targetid, begin_time, end_time):
    ...
    head = ['错误', '错误次数', '错误说明']
    des = "ORA-00600错误列表"
    sqlora600 = '''select substr(log_signature,1,position(',' in log_signature)-1),count(*),min(seq_no) from 
log_detail where begin_time between '{0}' and '{1}'  and target_id='{2}'
and upper(log_code) in ('ORA-00600','ORA-600') group by substr(log_signature,1,position(',' in log_signature)-1) 
order by 2 desc'''.format(begin_time, end_time, targetid)
    # print(sqlora600)
    sqlora600cursor = getValue(pg, sqlora600)
    sqlora600result = sqlora600cursor.fetchall()
    # print(sqlora600result)

    for resulttolist in sqlora600result:
        sqlora600result[sqlora600result.index(resulttolist)] = list(resulttolist)

    for row in sqlora600result:
        row[2] = getlogresult(pg, targetid, row[2])

    sqlresult = CommUtil.createTable(head, sqlora600result, des)
...
if __name__ == '__main__':
    ...
    try:
        ...
        ora600result = getora600list(pg, targetid, begintime, endtime)
        if ora600result:
            # print("msg=" + ora600result)
            sql6r = '''
begin;
delete from rpt_log_detail where rpt_id='{3}' and target_id='{2}' and rpt_log_code='ORA-00600';
...
select substr(log_signature,1,position(',' in log_signature)-1) log_signature,
 ...
 where begin_time between '{0}' and '{1}'
   and target_id='{2}'
   and upper(log_code) in ('ORA-00600','ORA-600') 
 group by substr(log_signature,1,position(',' in log_signature)-1) 
)
select '{3}' rptid,'{2}' target_id,'ORA-00600' log_code,a.log_signature,a.cnt,
