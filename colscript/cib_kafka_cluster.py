...
def get_kafka_info(pg, target_id, metric):
    sql = "select cib_value from p_normal_cib where index_id=1000001 and cib_name='brokers' and target_id='%s'" % target_id
    ...
    if result.code == 0:
        for row in result.msg:
            if row[0] in cs:
                cs.remove(row[0])
            #elif row[0].find('2904') == 0:
            elif row[0].find('290300006') == 0:
                if row[0] != target_id:
                    sql = "select uid from mgt_system where uid='%s' and use_flag" % (row[0])
                    result2 = relate_pg2(pg, sql)
